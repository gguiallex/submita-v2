...
export default async function EditarUAPage({
    params
}: {
    ...
    // 1. Busca os dados atuais da UA
    const ua = await prisma.unidadeAcademica.findUnique({
        where: { id: Number(id) }
    });

    // Se não existir, retorna 404
    if (!ua) return notFound();
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">
                            Editar Unidade Acadêmica
                        </h1>
                        <p className="text-slate-500 font-medium mt-1">Alterando informações de {ua.sigla}</p>
                    </div>
                </div>
            </header>
                ...
                <div className="bg-white border border-slate-200 rounded-[2.5rem] p-10 shadow-sm space-y-8">
                    ...
                    <div className="flex items-start gap-3 bg-amber-50 p-4 rounded-2xl border border-amber-100">
                        ...
                        <p className="text-[11px] text-amber-700 font-medium leading-relaxed">
                            <span className="font-bold">Atenção:</span> Ao alterar a sigla ou nome, as mudanças serão refletidas automaticamente em todos os <span className="font-bold">departamentos</span> vinculados a esta unidade acadêmica.
                        </p>
                    </div>
                </div>
